"""Viscous-flow PINN extension using the public pinn MLP component."""
...
def burgers_residual(model: MLP, xt: Tensor, viscosity: float) -> Tensor:
    """Return u_t + u u_x - viscosity u_xx at collocation points."""
    if xt.ndim != 2 or xt.shape[1] != 2:
        raise ValueError("xt must have shape (N, 2) with columns [x, t]")
    if viscosity <= 0:
        raise ValueError("viscosity must be > 0")

    coordinates = xt.clone().detach().requires_grad_(True)
    u = model(coordinates)
...
def main() -> None:
    ...
    interior = torch.rand(4_000, 2)
    interior[:, 0] = 2.0 * interior[:, 0] - 1.0

    x_initial = torch.linspace(-1.0, 1.0, 400).unsqueeze(1)
    initial = torch.cat((x_initial, torch.zeros_like(x_initial)), dim=1)
    ...
    for epoch in range(1, 1_501):
        ...
        residual = burgers_residual(model, interior, viscosity)
        physics_loss = residual.square().mean()
        initial_loss = (model(initial) - initial_target).square().mean()
        boundary_loss = 0.5 * (model(left).square().mean() + model(right).square().mean())
        ...
        if epoch in {1, 500, 1_000, 1_500}:
            print(
                ...
                f"initial={initial_loss.item():.3e} boundary={boundary_loss.item():.3e}"
            )
